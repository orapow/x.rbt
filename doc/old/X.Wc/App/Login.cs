...
namespace X.Wx.App
{
    ...
    class Login
    {
        ...
        {
            byte[] bytes = Wc2.SendGetRequest(_session_id_url);
            _session_id = Encoding.UTF8.GetString(bytes).Split(new string[] { "\"" }, StringSplitOptions.None)[1];
            bytes = Wc2.SendGetRequest(_qrcode_url + _session_id);
        ...
        public object LoginCheck()
        {
            ...
            byte[] bytes = Wc2.SendGetRequest(_login_check_url + _session_id);
        ...
        {
            byte[] bytes = Wc2.SendGetRequest(login_redirect + "&fun=new&version=v2&lang=zh_CN");
