...
namespace X.Wx.App
{
    ...
    class WXService
    {
        ...
        public JObject WxInit()
        {
            ...
            Cookie sid = Wc2.GetCookie("wxsid");
            Cookie uin = Wc2.GetCookie("wxuin");

            if (sid != null && uin != null)
            {
                init_json = string.Format(init_json, uin.Value, sid.Value);
                byte[] bytes = Wc2.SendPostRequest(_init_url + "&pass_ticket=" + Login.Pass_Ticket, init_json);
        ...
        {
            byte[] bytes = Wc2.SendGetRequest(_geticon_url + username);
        ...
        {
            byte[] bytes = Wc2.SendGetRequest(_getheadimg_url + usename);
            if (bytes.Length == 0) return null;
            return Image.FromStream(new MemoryStream(bytes));
        }
        /// <summary>
        ...
        {
            byte[] bytes = Wc2.SendGetRequest(_getcontact_url);
        ...
        public string WxSyncCheck()
        {
            ...
            if (sid != null && uin != null)
            {
                _synccheck_url = string.Format(_synccheck_url, sid.Value, uin.Value, sync_key, (long)(DateTime.Now.ToUniversalTime() - new System.DateTime(1970, 1, 1)).TotalMilliseconds, Login.SKey.Replace("@", "%40"), "e1615250492");

                byte[] bytes = Wc2.SendGetRequest(_synccheck_url +"&_=" + DateTime.Now.Ticks);
                if (bytes != null)
                {
        ...
        public JObject WxSync()
        {
            ...
            sync_json = string.Format(sync_json, uin.Value, sid.Value, _syncKey.Count, sync_keys, (long)(DateTime.Now.ToUniversalTime() - new System.DateTime(1970, 1, 1)).TotalMilliseconds, Login.SKey);

            if (sid != null && uin != null)
            {
                byte[] bytes = Wc2.SendPostRequest(_sync_url + sid.Value + "&lang=zh_CN&skey=" + Login.SKey + "&pass_ticket=" + Login.Pass_Ticket, sync_json);
        ...
        public void SendMsg(string msg, string from, string to, int type)
        {
            ...
            if (sid != null && uin != null)
            {
                msg_json = string.Format(msg_json, sid.Value, uin.Value, msg, from, to, type, Login.SKey, DateTime.Now.Millisecond, DateTime.Now.Millisecond, DateTime.Now.Millisecond);

                byte[] bytes = Wc2.SendPostRequest(_sendmsg_url + sid.Value + "&lang=zh_CN&pass_ticket="+Login.Pass_Ticket, msg_json);
